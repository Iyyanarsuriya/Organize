...
// --- SECTOR MAP ---
const TABLE_MAP = {
    operations: 'operations_vehicle_logs',
    manufacturing: 'operations_vehicle_logs'
};

// --- OPERATIONS SECTOR ---
const OperationsVehicleLogModel = {
    create: async (data) => {
        ...
        const [res] = await db.query(
            `INSERT INTO operations_vehicle_logs (user_id, vehicle_name, vehicle_number, driver_name, member_id, in_time, out_time, start_km, end_km, expense_amount, income_amount, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    getAllByUserId: async (user_id) => {
        const [rows] = await db.query(`SELECT * FROM operations_vehicle_logs WHERE user_id = ? ORDER BY in_time DESC, created_at DESC`, [user_id]);
        return rows;
    }
};
...
const getSectorModel = (sector) => {
    return OperationsVehicleLogModel;
};
...
const getAllByUserId = async (user_id, sector = 'operations') => {
...
const updateResult = async (id, data) => {
    ...
    await db.query(
        `UPDATE operations_vehicle_logs SET vehicle_name = ?, vehicle_number = ?, driver_name = ?, member_id = ?, in_time = ?, out_time = ?, start_km = ?, end_km = ?, expense_amount = ?, income_amount = ?, notes = ? WHERE id = ?`,
...
const deleteResult = async (id) => {
    await db.query(`DELETE FROM operations_vehicle_logs WHERE id = ?`, [id]);
};
