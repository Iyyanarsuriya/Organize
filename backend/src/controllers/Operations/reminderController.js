...
exports.getReminders = async (req, res) => {
    try {
        ...
        console.error("Error in Operations getReminders:", error);
...
exports.createReminder = async (req, res) => {
    try {
        ...
        console.error("Error in Operations createReminder:", error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
exports.updateReminder = async (req, res) => {
    ...
    try {
        ...
        console.error("Error in Operations updateReminder:", error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
exports.deleteReminder = async (req, res) => {
    ...
    try {
        ...
        console.error("Error in Operations deleteReminder:", error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
exports.sendMissedAlert = async (req, res) => {
    ...
    try {
        ...
        res.json({ message: `Operations report sent for ${date || 'today'}`, ...result });
    } catch (error) {
        console.error('Operations missed alert error:', error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
